        ...
        with open('vol.txt', 'r+') as temp:
            ...
            try:
                if text[0] == 0.0:
                    ...
                    time = 0.0
                    temp = float(text[0])
                    os.system("ffmpeg -f wav -ss " + str(time) + " -i " + file + " -t " + str(temp) + " -c copy out/Youtube_dataset/" + dir + "/" + title + "_" + str(clip) + ".wav")
                    time = text[1]
                    clip += 1
            except:
                print("no silence")
                os.system("ffmpeg -f wav -i " + file + " -c copy out/Youtube_dataset/" + dir + "/" + title + "_" + str(clip) + ".wav")
            totalTime = 0;
            while i < len(text):
                j = 2
                try:
